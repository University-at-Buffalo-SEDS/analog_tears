...
HAL_StatusTypeDef AD7193_Reset(AD7193_HandleTypeDef *hadc7193)
{
    ...
    HAL_StatusTypeDef status = HAL_SPI_Transmit(hadc7193->hspi, reset_bytes, sizeof(reset_bytes), AD7193_SPI_TIMEOUT);
...
HAL_StatusTypeDef AD7193_WriteReg(AD7193_HandleTypeDef *hadc7193, uint8_t regAddr, uint32_t data)
{
    ...
    AD7193_SPI_Start(hadc7193);
    HAL_StatusTypeDef status = HAL_SPI_Transmit(hadc7193->hspi, tx_buffer, sizeof(tx_buffer), AD7193_SPI_TIMEOUT);
    AD7193_SPI_End(hadc7193);
                                                      ...
                                                      float *voltage)
{
    ...
    *voltage = (((float)rawData / 8388608.0f) - 1.0f) * (hadc7193->vref / (float)gain);
...
/**
  * @brief  Set the AD7193 cs_pin LOW. This is required for the interrupt to work.
  */
